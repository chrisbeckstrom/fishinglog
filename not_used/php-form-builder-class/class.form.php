...
STR;
		...
		if(!empty($this->ckeditorIDArr) && empty($this->preventCKEditorLoad))
			$str .= "\t\t<script type='text/javascript' src='{$this->jsIncludesPath}/ckeditor/ckeditor.js'></script><script type='text/javascript' src='{$this->jsIncludesPath}/ckeditor/adapters/js/jquery.js'></script>\n";
	...
	public function renderJS($returnString=false) {
		...
		if(!empty($_SESSION["pfbc-instances"]) && array_key_exists($this->attributes["id"], $_SESSION["pfbc-instances"])) {
			...
			if(empty($form->synchronousResources)) {
				...
				if(!empty($form->jqueryDateRangeIDArr))
					$str .= file_get_contents("{$form->phpIncludesPath}/jquery/plugins/daterangepicker/daterangepicker.js/jquery.js");
				if(!empty($form->jqueryColorIDArr))
				...
				if(!empty($form->jqueryDateRangeIDArr))
					$str .= "\n<script type='text/javascript' src='{$form->jsIncludesPath}/jquery/plugins/daterangepicker/daterangepicker.js/jquery.js'></script>";
				if(!empty($form->jqueryColorIDArr))
...
STR;
	...
	var mapoptions_$latlngID = { zoom: $latlngZoom, center: latlng_$latlngID, mapTypeId: google.maps.MapTypeId.TERRAIN, mapTypeControl: true }
	...
	google.maps.event.addListener(marker_$latlngID, "dragend", function() {
		...
		var lat = latlng.lat();
		var lng = latlng.lng();
		document.getElementById("$latlngID").value = "Latitude: " + lat.toFixed(6) + ", Longitude: " + lng.toFixed(6);
